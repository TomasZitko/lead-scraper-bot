...
class DatabaseManager:
    ...
    def _apply_schema_migrations(self):
        """Ensure database is up to date with latest schema changes"""
        try:
            if not self._column_exists('area_progress', 'quality_score'):
                self.cursor.execute(
                    "ALTER TABLE area_progress ADD COLUMN quality_score INTEGER DEFAULT 0"
                )
                self.conn.commit()
        except sqlite3.OperationalError as e:
            logging.warning(f"Schema migration failed: {e}")

    def _column_exists(self, table: str, column: str) -> bool:
        """Check if a column exists in a table"""
        self.cursor.execute(f"PRAGMA table_info({table})")
        columns = [row['name'] if isinstance(row, sqlite3.Row) else row[1] for row in self.cursor.fetchall()]
